...
function renderMihomoYaml(proxies: ProxyNode[], requestUrl: URL, template?: RoutingTemplateConfig) {
  ...
  const document = {
    ...
    "proxy-groups": renderedGroups,
    ...(hasRuleProviders ? { "rule-providers": ruleProviders } : {}),
    rules: config.rules && config.rules.length > 0 ? config.rules : [`MATCH,${fallbackPolicy}`],
  };
...
function renderTemplateProxyGroups(proxies: ProxyNode[], groupTemplates: TemplateProxyGroup[]): RenderedProxyGroup[] {
  ...
  const includedGroupNames = new Set<string>();
  let progressed = true;
  while (progressed) {
    progressed = false;
    for (const group of expanded) {
      if (includedGroupNames.has(group.name)) continue;
      const usable = group.proxies.some(
        (name) => nodeNameSet.has(name) || includedGroupNames.has(name) || allowedLiterals.has(name),
      );
      if (usable) {
        includedGroupNames.add(group.name);
        progressed = true;
      }
    }
  }
  ...
  return expanded.flatMap((group) => {
    const proxyEntries = uniqueStrings(
    ...
    return [stripUndefined({ ...rest, proxies: proxyEntries }) as RenderedProxyGroup];
  });
}
